...
monomorphic functions that take no arguments, and the return type must be one of the following:

- `()`
- `ds::io::Result<()>`
- `!`

> Note: The test mode is enabled when using `divescript test`.
...
> **Note**: The test harness supports the `--include-ignored` flag to
