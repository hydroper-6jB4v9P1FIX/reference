...
When the crate's debug executable is passed into GDB[^ds-gdb], `print bob` will display:

```text
"Bob" is 10 years old.
```

[^ds-gdb]: Note: This assumes you are using the `ds-gdb` script which configures pretty-printers for standard library types like `String`.
