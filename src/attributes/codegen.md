...
> ***Note***: The `dsc` compiler automatically inlines functions based on
...
> Note: `dsc` has a default set of features enabled for each target and CPU.
...
The `track_caller` attribute may be applied to any function with [`"DS"` ABI][ds-abi]
...
> the [`core::intrinsics::caller_location`] intrinsic implemented by `dsc`.
...
> Note: The aforementioned shim for function pointers is necessary because `dsc` implements
...
[`-C target-cpu`]: ../../dsc/codegen-options/index.html#target-cpu
[`-C target-feature`]: ../../dsc/codegen-options/index.html#target-feature
...
[ds-abi]: ../items/external-blocks.md#abi
