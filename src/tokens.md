...
`U+005C` (`\`) character. It is equivalent to a `u32` unsigned 32-bit integer
_number literal_.
...
below. The type of a byte string literal is `Bytes`.
...
Note that `-1i32`, for example, is analyzed as two tokens: `-` followed by `1i32`.
...
[Loop labels] use LABEL_TOKEN tokens. Any
...
| `*`    | Star        | [Multiplication][arith], [Macro Kleene Matcher][macros], [Use wildcards]
...
| `->`   | RArrow      | [Function return type][functions], [Closure return type][closures], [Function type]
| `=>`   | FatArrow    | [Match arms][match], [Macros]
| `#`    | Pound       | [Attributes]
| `$`    | Dollar      | [Macros]
| `?`    | Question    | [Question mark operator][question], [Macro Kleene Matcher][macros]
| `~`    | Tilde       | The tilde operator is unused
