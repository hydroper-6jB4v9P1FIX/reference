...
If the type of the `self` parameter is specified, it is limited to `Self`.
...
name of the associated type, `Params` is a comma-separated list of type parameters, `Bounds` is a plus-separated list of trait bounds
...
has an associated type `Item` from a trait `Trait` with the generics `<T>` , the
type can be named like `<Thing as Trait>::Item<str>`. In this case, `str` will be used wherever `T` appears in the associated
...
implementation of `Container` for the built-in array type:
...
associated types initiative repository](https://ds-lang.github.io/generic-associated-types-initiative/explainer/required_bounds.html).
